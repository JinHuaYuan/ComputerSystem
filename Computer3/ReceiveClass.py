...
class Receive(tk.Frame):
    def __init__(self, master=None):
        ...
        self.receive_date=tk.StringVar()    #領取日期
        self.receive_person = tk.StringVar()  # 領取人
    ...
    def check_date(self):
        try:
           # datetime.datetime.strptime(self.back_date.get(), '%Y%m%d')
            ...
            receive_date=datetime.datetime.strptime(self.receive_date.get(),'%Y%m%d')

            receive_days=(receive_date-nowdate).days
            if(receive_days<0):
    ...
    def createPage(self):
        ...
        def search():
            dict = sql.WeiXiuSql().search_tz(self.pc_no.get().strip(),'15')
            if (len(dict) > 0):
                self.pc_nm.set(dict["pc_name"])
                self.back_date.set(dict["back_date"])
                ...
                messagebox.showinfo("", "沒有此設備編號或無回廠確認")
        ...
        def save():
            ...
            flag=self.checknull()
            if flag:
                sql.WeiXiuSql().updatereceive(self)
                # reset()
        ...
        tk.Label(frm2, text="回廠日期").grid(row=2, column=0)
        tk.Label(frm2, text="領取日期").grid(row=3, column=0)
        tk.Label(frm2, text="領取人").grid(row=4, column=0)
       ...
       # txt_pc_no.config(state="disable")
        ...
        txt_back_factory_date.config(state="disable")
        txt_back_factory_date.grid(row=2, column=1, sticky=tk.W, padx=10)
        ...
        txt_receive_date = tk.Entry(frm2, textvariable=self.receive_date)
        txt_receive_date.grid(row=3, column=1, sticky=tk.W, padx=10)




        def masterinfo(event):
            dict=sqlbase.CONDB().getEmpInfo(self.receive_person.get()[0:5])
            if(len(dict)>0):
        ...
        txt_receive_person.grid(row=4, column=1, sticky=tk.W, padx=10)
