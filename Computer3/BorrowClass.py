...
class Borrow(tk.Frame):
    ...
    def checknull(self):
        if not self.con_person.get():
            ...
            return  False
        if not self.len_date.get():
            messagebox.showinfo("","借出日期不允許為空")
        ...
        if not self.back_date.get():
            messagebox.showinfo("","計劃歸還日期不允許為空")
            return False
        if not self.checkdate(self.len_date.get()):
            messagebox.showinfo("", "借出日期應為八位有效數字")
            return False
        if not self.checkdate(self.back_date.get()):
            messagebox.showinfo("", "計劃歸還日期應為八位有效數字")
            return False
        return True
    ...
    def createPage(self):
        ...
        def search():
            dict=Borrowsql.BorrowSql().search(self.pc_no.get().strip())
            if (len(dict) > 0):
                ...
                self.len_date.set(str(datetime.datetime.now().strftime("%Y%m%d")))
        ...
        def save():
            ...
            if flag:
                self.upddate.set(str(datetime.datetime.now().strftime("%Y%m%d%H%M%S")))
                Borrowsql.BorrowSql().insert(self)
        ...
        def masterinfo(event):
            dict=sqlbase.CONDB().getEmpInfo(self.con_person.get()[0:5])
            if(len(dict)>0):
        ...
        txt_con_emp.bind('<Return>', masterinfo)
        txt_con_emp.bind('<FocusOut>', masterinfo)
